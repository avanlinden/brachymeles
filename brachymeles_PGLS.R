...
plot(ladderize(skinkTree), cex = 0.5)
...
treeSpecEdit %>% 
  ...
  dplyr::select(species) %>% 
  unique() #ok so for whatever reason only 8 are in the tree...I assume it's spelling

#there is no B. vermis because it's not in the tree -- no tissue samples exist and restricted access to island
...
# Load the CLEANED and CORRECT data:
skinkDat <- as.tibble(read.csv("cleaned_Brachymeles_data.csv"))
...
skinkDat %>% 
  ggplot(aes(log10(SVLDig), log10(headCirc))) +
  geom_point()
...
tail <- pgls(log10(midtailirc) ~ log10(SVLDig), data = skinkComp, lambda = "ML")
summary(tail) 
